-- Update the publish_product function to allow admins to bypass subscription checks
...
BEGIN
  ...
  -- 5. For non-admin users, check for an active subscription and get the product limit
  SELECT
    is_active,
    product_limit
  INTO
  ...
  -- 6. If no active subscription, raise an exception
  ...
  -- 9. Check if the update succeeded (i.e., if a row was found and updated)
  IF NOT FOUND THEN
    RAISE EXCEPTION 'PRODUCT_NOT_FOUND_OR_NOT_OWNED';
  END IF;
