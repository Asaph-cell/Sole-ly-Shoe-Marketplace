...
export const ShoeSizeChart = ({ selectedSize, availableSizes }: ShoeSizeChartProps) => {
            ...
            <DialogContent className="max-w-lg max-h-[80vh] overflow-auto">
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {shoeSizeChart.map((row, idx) => {
                                const isSelected = row.eu === selectedSize;
                                const isAvailable = !availableSizes || availableSizes.length === 0 || availableSizes.includes(row.eu);

                                return (
                                    <tr
                                        key={idx}
                                        className={`border-b transition-colors ${isSelected ? "bg-primary/20 font-medium" :
                                            isAvailable ? "hover:bg-muted/30" : "opacity-40"
                                            }`}
                                    >
                ...
                {availableSizes && availableSizes.length > 0 && (
                    <div className="mt-4 p-3 bg-muted rounded-lg text-sm">
                        ...
                        <p className="text-primary font-semibold">EU {availableSizes.join(", ")}</p>
                    </div>
                )}

                <div className="mt-4 p-3 bg-muted/50 rounded-lg text-xs text-muted-foreground">
                    <p><strong>Tip:</strong> Measure your foot length in centimeters (CM) for the most accurate fit.</p>
                </div>
            </DialogContent>
        </Dialog>
    );
